...
<head>
  ...
  <!-- 네이버 지도 서비스 라이브러리: geocode() 호출을 위해 services 파라미터 필요 -->
  <script src="https://openapi.map.naver.com/openapi/v3/maps.js?libraries=services&ncpClientId=YOUR_CLIENT_ID"></script>
  <style>
  ...
  <script>
    ...
    // 주소를 위도/경도로 변환한 뒤 폼을 제출하는 함수
    function geocodeAndSubmit(form) {
      // 주소 입력값을 읽는다. input name="address" 가 없으면 그대로 제출
      var address = form.address ? form.address.value : '';
      if (!address) return true;
      naver.maps.Service.geocode({ query: address }, function (status, response) {
        if (status === naver.maps.Service.Status.OK && response.v2.addresses.length > 0) {
          var result = response.v2.addresses[0];
          // 좌표를 hidden 필드에 채운다. 네이버 API: result.y = latitude, result.x = longitude
          if (form.latitude)  form.latitude.value  = result.y;
          if (form.longitude) form.longitude.value = result.x;
          form.submit();
        } else {
          alert('주소를 좌표로 변환하는 데 실패했습니다. 주소를 다시 확인해주세요.');
        }
      });
      return false; // 비동기 호출로 기본 제출 방지
    }
    </script>
</head>
<body>
<div class="wrap">
  ...
  <section>
    <h2>가게 등록</h2>
    <form method="post" action="<%=ctx%>/admin.do" class="mb-2" onsubmit="return geocodeAndSubmit(this);">
      ...
      위도: <input type="text" name="latitude" placeholder="위도 (예: 37.5665)">
      경도: <input type="text" name="longitude" placeholder="경도 (예: 126.9780)">
